  ...
  return (
    <section className="rounded-2xl border border-border bg-surface p-3">
        ...
        <button
          ...
          onClick={() => {
            if (window.confirm(`Remove ${entry.exercise.name}?`)) {
              void removeExercise({ workoutExerciseId: entry.workoutExerciseId })
            }
          }}
      ...
      <div className="mt-1 flex flex-col gap-1">
        {entry.sets.map((set) => (
          <SetRow key={set._id} set={set} record={record} />
        ))}
      </div>

      <button
        type="button"
        onClick={() => void addSet({ workoutExerciseId: entry.workoutExerciseId })}
...
function SetRow({
  ...
  return (
    ...
    >
      ...
      <button
        type="button"
        onClick={() => void updateSet({ setId: set._id, isWarmup: !set.isWarmup })}
      ...
      <button
        type="button"
        onClick={() => void removeSet({ setId: set._id })}
